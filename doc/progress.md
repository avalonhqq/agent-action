...
| 5   | RAG 知识表示与 Chunk        | 进行中 | 2026-07-25 |            | Loader 等自动完成；重点学习结构化分块与 Small-to-Big               |
...
| Step | 内容 | 状态 | 验收目标 |
|---|---|---|---|
| 5A | 文档入库契约与自动工程底座 | 进行中 | PDF/DOCX/Markdown/TXT 可追踪入库，重复文件幂等 |
| 5B | 结构化 Chunk 与 Small-to-Big | 待开始 | Child 可召回、Parent 可还原，标题和表格语义不丢失 |
| 5C | Chunk 数据集、评估与知识库调试接口 | 待开始 | 固定样本比较策略，失败样本可定位到文档和结构 |
