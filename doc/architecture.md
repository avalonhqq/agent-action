...
业务知识请求由服务端根据意图中的业务域和当前用户身份生成检索条件，客户端不能自行声明管理员权限。检索到 Parent 证据后，系统使用`grounded_support:v4`生成严格Claims JSON；v3提供DeepSeek完整格式样例和一次受限结构重试，v4增加最小直接证据选择，随后依次完成Pydantic结构校验、证据ID白名单和声明支持度验证。只有`pass`才发布答案，没有证据、检索失败或验证失败均返回确定性边界答复。普通聊天仍支持SSE逐Token流式输出，知识回答则完整验证后发布，避免先泄露未经校验内容。
